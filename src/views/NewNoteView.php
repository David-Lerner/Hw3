...
class NewNoteView extends View {
  ...
  public function render($data){
    ...
    <form action="index.php" method="GET">
        <input type="hidden" name="c" value="NewNoteController" />
        <input type="hidden" name="m" value="submitAction" />
        <input type="hidden" name="arg1" value=<?=$current_list?> />
        Title:<input type="text" name="arg2" />
        <br/>Note<br/>
        <textarea name="arg3" rows="6" cols="32" ></textarea>
        <div class="submission"><input type="submit" value="Save"></div>
	  </form>  
    </div>
    <?php
